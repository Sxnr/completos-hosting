...
export default async function minecraftRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{
    ...
    const { name, description, software, version, edition, port, ramMb } = request.body

    if (!name || !software || !version || !edition)
      return reply.status(400).send({ error: 'missing_fields', message: 'name, software, version y edition son requeridos' })

    try {
      const instance = await fastify.minecraft.createInstance({
        name, description, software, version, edition, port,
        ramMb:     ramMb || 1024,
        createdBy: user.id,
      })
  ...
  fastify.get<{ Params: { id: string } }>('/api/minecraft/:id', {
    preHandler: [fastify.authenticate],
  }, async (request, reply) => {
    ...
    try {
      ...
      players     = mem.players
    } catch { /* instancia no en memoria — es offline */ }

    return { ...row, status, playerCount, players }
  })

  // ── DELETE /api/minecraft/:id — eliminar instancia ────
  fastify.delete<{ Params: { id: string } }>('/api/minecraft/:id', {
    preHandler: [fastify.authenticate],
  }, async (request, reply) => {
    ...
    try {
      ...
      return reply.status(500).send({ error: 'delete_error', message: err.message })
    }
  })

  // ── POST /api/minecraft/:id/start ─────────────────────
  fastify.post<{ Params: { id: string } }>('/api/minecraft/:id/start', {
    preHandler: [fastify.authenticate],
  }, async (request, reply) => {
    const id = parseInt(request.params.id)
    try {
      ...
      return reply.status(500).send({ error: 'start_error', message: err.message })
    }
  })

  // ── POST /api/minecraft/:id/stop ──────────────────────
  fastify.post<{ Params: { id: string } }>('/api/minecraft/:id/stop', {
    preHandler: [fastify.authenticate],
  }, async (request, reply) => {
    const id = parseInt(request.params.id)
    try {
      ...
      return reply.status(500).send({ error: 'stop_error', message: err.message })
    }
  })

  // ── POST /api/minecraft/:id/restart ───────────────────
  fastify.post<{ Params: { id: string } }>('/api/minecraft/:id/restart', {
    preHandler: [fastify.authenticate],
  }, async (request, reply) => {
    const id = parseInt(request.params.id)
    try {
      ...
      return reply.status(500).send({ error: 'restart_error', message: err.message })
    }
  })
  ...
  fastify.post<{
    ...
    try {
      ...
      if (!instance.isRunning)
        return reply.status(400).send({ error: 'not_running', message: 'El servidor no está corriendo' })

      instance.sendCommand(command)
      return { success: true }
    } catch (err: any) {
      return reply.status(500).send({ error: 'command_error', message: err.message })
    }
  })

  // ── GET /api/minecraft/:id/console — historial ────────
  fastify.get<{ Params: { id: string } }>('/api/minecraft/:id/console', {
    preHandler: [fastify.authenticate],
  }, async (request, reply) => {
    const id = parseInt(request.params.id)
    try {
  ...
  fastify.put<{
    Params: { id: string }
    Body:   { properties: Record<string, unknown>; ramMb?: number; javaFlags?: string }
    ...
    fs.writeFileSync(path.join(instanceDir, 'server.properties'), lines.join('\n'))

    return { success: true, message: 'Configuración guardada. Reinicia el servidor para aplicar cambios.' }
  })
  ...
  fastify.get<{
    ...
    if (!targetDir.startsWith(baseDir))
      return reply.status(403).send({ error: 'forbidden', message: 'Path no permitido' })
    ...
    const files = entries.map(entry => ({
      ...
      size:     entry.isFile()
        ...
        : null,
      modified: fs.statSync(path.join(targetDir, entry.name)).mtime.toISOString(),
    }))
  ...
  fastify.get<{
    ...
    try {
      ...
      return reply.status(500).send({ error: 'fetch_error', message: err.message })
    }
  })
  ...
  fastify.post<{
    ...
    fastify.minecraft.downloadJar(row.software, row.version)
...
async function _fetchVersions(software: string): Promise<string[]> {
  switch (software) {
    ...
    case 'bedrock':
    case 'pocketmine':
      return ['1.21.4', '1.21.3', '1.21.1', '1.20.6', '1.20.4', '1.20.2', '1.20.1', '1.19.4', '1.18.2', '1.17.1', '1.16.5']
