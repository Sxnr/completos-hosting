...
function ConsolePanel({ instance }: { instance: McInstance }) {
  const { lines, status, playerCount, players, connected, sendCommand, clearConsole } =
    ...
    <div className="mc-console-panel">
      ...
      <div className="mc-console-header">
        <div className="mc-console-meta">
          <span className={`status-dot ${STATUS_CLASS[status] ?? 'status-dot--offline'}`} />
          <span className="mc-console-status">{STATUS_LABEL[status] ?? status}</span>
        ...
        <button className="mc-btn mc-btn--ghost mc-btn--sm" onClick={clearConsole}>
          Limpiar
        </button>
      </div>
      ...
      <div className="mc-console-input-row">
        ...
        <input
          ...
          placeholder={status === 'online' ? 'Escribe un comando...' : 'El servidor no está corriendo'}
...
function CreateModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      const res  = await api('/api/minecraft', { method: 'POST', body: JSON.stringify(form) })
      const data = await res.json()
      if (!res.ok) throw new Error(data.message ?? data.error ?? 'Error al crear')
        ...
        <form className="mc-modal-form" onSubmit={handleSubmit}>
          ...
          <div className="mc-form-grid">
            <div className="mc-form-row">
              ...
              <select value={form.software} onChange={e => set('software', e.target.value)}>
                {['paper','vanilla','spigot','fabric','forge','purpur','neoforge'].map(s => (
            ...
            <div className="mc-form-row">
              <label>RAM (MB)</label>
              <select value={form.ramMb} onChange={e => set('ramMb', parseInt(e.target.value))}>
...
export default function MinecraftPage() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (data.error === 'start_error' && data.message?.includes('JAR_DOWNLOADING')) {
        const inst = instances.find(i => i.id === id)
        if (inst) setDownloadModal({ instanceId: id, software: inst.software, version: inst.version })
        return
      }
      ...
      <div className="dashboard-content">
        ...
        <div className="page-header">
          <div>
            ...
            <p className="page-subtitle">Gestiona tus servidores de Minecraft</p>
          </div>
          <button
        ...
        {!loading && instances.length === 0 && (
          <div className="mc-empty-state">
            ...
            <button className="mc-btn mc-btn--primary" onClick={() => setShowCreate(true)}>
