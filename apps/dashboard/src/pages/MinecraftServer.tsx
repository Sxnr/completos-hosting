...
function NavItem({
  ...
  return (
    <div
      className={`srv-nav-item${active ? " active" : ""}`}
      onClick={onClick}
      style={{
        display: "flex",
        alignItems: "center",
...
const PROP_FIELDS = [
  ...
  { key: "enable-command-block", label: "Bloques de comandos", type: "bool" },
  {
    ...
    label: "Paquete de recursos requerido",
    type: "bool",
  },
...
export default function MinecraftServer() {
  ...
  const fetchServer = useCallback(async () => {
    try {
      ...
      if (s) {
        setServer(s);
        if (!nameVal) setNameVal(s.nombre);
      }
    } catch {}
  }, [id]);
  ...
  const fetchStats = useCallback(async () => {
    try {
      const r = await api.get(`/minecraft/${id}/stats`);
      setStats(r.data);
    } catch {}
  }, [id]);
  const fetchPlayers = useCallback(async () => {
    try {
      const r = await api.get(`/minecraft/${id}/players`);
      setPlayers(r.data);
    } catch {}
  }, [id]);
  ...
  const fetchProps = useCallback(async () => {
    try {
      const r = await api.get(`/minecraft/${id}/files`, {
  ...
  const doAction = async (a: "start" | "stop" | "restart") => {
    setAction(a);
    try {
      await api.post(`/minecraft/${id}/${a}`);
      setTimeout(fetchStats, 2000);
    } catch (e: any) {
      alert(e.response?.data?.error || `Error: ${a}`);
    } finally {
      setAction("");
    }
  };
  ...
  const sendCmd = async () => {
    if (!cmd.trim()) return;
    try {
      await api.post(`/minecraft/${id}/command`, { command: cmd });
      setCmd("");
  ...
  const slug = server
    ? server.nombre
        ...
        .slice(0, 32)
    : "";
  const serverIP = server ? `${slug}.completohosting.lat` : "";
    ...
    <>
      ...
      >
        ...
        >
          {/* Server identity */}
          <div
            style={{ padding: "16px 14px", borderBottom: "1px solid #21262d" }}
          >
            <div
              style={{
                display: "flex",
            ...
            >
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                {editName ? (
                  ...
                  >
                    {server.nombre}{" "}
                    <span style={{ color: "#484f58", fontSize: 11 }}>✏️</span>
                  </div>
                )}
                <div
                  style={{
          ...
          <nav style={{ flex: 1, padding: "10px 8px", overflowY: "auto" }}>
            ...
            <NavItem
              icon="👥"
              label="Jugadores"
              active={tab === "jugadores"}
              onClick={() => setTab("jugadores")}
              badge={players?.players.length}
            />
            <NavItem
              icon="⚙️"
              label="Opciones"
                ...
                style={{
                  ...
                  background: isRunning
                    ...
                    : isStarting
                      ...
                      : "#f87171",
                  animation:
                    isRunning || isStarting ? "pulse-dot 2s infinite" : "none",
                }}
              />
              <span
                style={{
              ...
              {!isRunning && !isStarting && (
                <button
                  ...
                  style={{
                    ...
                    fontFamily: "monospace",
                    cursor: "pointer",
                    boxShadow: "0 4px 12px rgba(35,134,54,.4)",
                    display: "flex",
                    alignItems: "center",
                    gap: 6,
                  }}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 24 }}>
              ...
              <div style={{ animation: "fadeUp .3s ease", maxWidth: 860 }}>
                ...
                >
                  {[
                    ...
                    <div
                      key={card.l}
                      onClick={card.copy ? () => copyText(serverIP) : undefined}
                      style={{
                        background: "linear-gradient(135deg,#161b22,#1e2535)",
                        border: "1px solid #21262d",
                        borderRadius: 12,
                        padding: 16,
                        cursor: card.copy ? "pointer" : "default",
                      }}
                    >
                      <div
                        style={{
                  ...
                  >
                    ...
                    >
                      {[
                        {
                          l: "CPU",
                          val: `${stats.cpuPercent.toFixed(1)}%`,
                          pct: stats.cpuPercent,
                          c: stats.cpuPercent > 80 ? "#f87171" : "#34d399",
                        },
                        {
                          l: "RAM",
                          val: `${fmt(stats.memUsage)}/${fmt(stats.memLimit)}`,
                          pct: (stats.memUsage / stats.memLimit) * 100,
                          c: "#a78bfa",
                        },
                      ].map((st) => (
                        <div key={st.l}>
                ...
                {isRunning && players && (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {players.players.length}/{players.maxPlayers}
                      </span>
                    </div>
            ...
            {tab === "consola" && (
              ...
              >
                ...
                >
                  {logs || (
                    <span style={{ color: "#484f58" }}>Esperando logs...</span>
                  )}
                </div>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <input
                    ...
                    placeholder="Ejecutar comando en el servidor..."
                    style={{ ...inp, flex: 1 }}
                  />
                  <button
                    className="srv-btn"
                  ...
                  {filePath
                    ...
                    .map((seg, i, arr) => (
                      ...
                      >
                        ...
                        <span
                          style={{
                            color: i === arr.length - 1 ? "#e6edf3" : "#58a6ff",
                            cursor: "pointer",
                          }}
                          onClick={() =>
                            fetchFiles(arr.slice(0, i + 1).join("/"))
                          }
                        >
                          {seg}
                        </span>
                    ...
                    files.map((f, i) => (
                      <div
                        ...
                        onClick={() =>
                          ...
                          fetchFiles(
                            f.path ||
                              (filePath ? `${filePath}/${f.name}` : f.name),
                          )
                        }
                        style={{
                          display: "grid",
                      ...
                      >
                        ...
                        >
                          ...
                          <span
                            style={{
                              ...
                              textOverflow: "ellipsis",
                              whiteSpace: "nowrap",
                              color: f.type === "dir" ? "#58a6ff" : "#e6edf3",
                            }}
                          >
                            {f.name}
                          </span>
                        ...
                        <span style={{ color: "#6b7280" }}>
                          {f.modified
                            ? new Date(f.modified).toLocaleDateString("es-CL")
                            : "—"}
                        </span>
                        <span>
                          {f.type === "file" && (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                api
                                  .delete(`/minecraft/${id}/files`, {
                                    params: { path: f.path },
                                  })
                                  .then(() => fetchFiles(filePath));
                              }}
                              style={{
                                background: "rgba(248,113,113,.1)",
                                color: "#f87171",
              ...
              <div style={{ animation: "fadeUp .3s ease", maxWidth: 680 }}>
                ...
                >
                  ...
                  {!players || players.players.length === 0 ? (
                    ...
                    >
                      {isRunning ? "Nadie conectado" : "Servidor offline"}
                    </div>
                  ) : (
                  ...
                  {PROP_FIELDS.map((field) => (
                    ...
                    >
                      ...
                      <div style={{ flexShrink: 0 }}>
                        {field.type === "bool" && (
                          <Toggle
                            value={(props[field.key] ?? "false") === "true"}
                            onChange={(v) =>
                              setProps((p) => ({
                                ...p,
                            ...
                            <button
                              onClick={() =>
                                setProps((p) => ({
                                  ...p,
                                  [field.key]: String(
                                    Math.max(0, Number(p[field.key] ?? 0) - 1),
                                  ),
                                }))
                              }
                              style={{
                  ...
                  >
                        ...
                        onClick={async () => {
                          try {
                            const r = await api.get(
                              `/minecraft/${id}/download-world`,
                              { responseType: "blob" },
                            );
                            const url = URL.createObjectURL(r.data);
                            const a = document.createElement("a");
                            a.href = url;
                            a.download = `${server.nombre}-world.zip`;
                            a.click();
                          } catch {
                    ...
                    >
                      <div>
                        ...
                        >
                          Importar .zip para reemplazar world/
                        </div>
                      </div>
                      <label
                        style={{
                    ...
                    >
                      ⚠️ Detén el servidor antes de subir un mundo para evitar
                      corrupción.
                    </div>
                  </div>
                </div>
