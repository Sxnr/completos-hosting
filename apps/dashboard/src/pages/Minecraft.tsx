...
const MC_VERSIONS_GROUPED = [
  ...
  },
  {
    ...
    versions: ["1.20.6", "1.20.4", "1.20.2", "1.20.1", "1.20"],
  },
  {
  ...
  { label: "1.15.x — Buzzy Bees", versions: ["1.15.2", "1.15.1", "1.15"] },
  {
    ...
    versions: ["1.14.4", "1.14.3", "1.14.2", "1.14"],
  },
  ...
  { label: "1.9.x — Combat Update", versions: ["1.9.4", "1.9.2", "1.9"] },
  {
    ...
    versions: ["1.8.9", "1.8.8", "1.8.7", "1.8"],
  },
  {
    label: "1.7.x — The Update that Changed the World",
    versions: ["1.7.10", "1.7.5", "1.7.2"],
  },
  { label: "1.6.x — Horse Update", versions: ["1.6.4", "1.6.2", "1.6.1"] },
  { label: "1.5.x — Redstone Update", versions: ["1.5.2", "1.5.1", "1.5"] },
  {
    label: "1.4.x — Pretty Scary Update",
    versions: ["1.4.7", "1.4.6", "1.4.2"],
  },
...
export default function Minecraft() {
  ...
  const [logs, setLogs] = useState<{ id: string; content: string } | null>(
    null,
  );
    ...
    await Promise.all(
      running.map(async (s) => {
        try {
          ...
          setExtras((prev) => ({
            ...prev,
            [s.id]: { stats: statsR.data, players: playersR.data },
          }));
        } catch {}
      }),
    );
  ...
  useEffect(() => {
    if (servers.length) fetchExtras(servers);
  }, [servers.length]);
  ...
  const handleAction = async (
    ...
    if (
      action === "delete" &&
      !confirm("¿Eliminar este servidor? No se puede deshacer.")
    )
      return;
  ...
  const handleLogs = async (id: string) => {
    try {
      ...
      setLogs({ id, content: r.data.logs });
    } catch {
  ...
  const loadFiles = async (id: string, p: string) => {
    ...
    try {
      const r = await api.get(
        `/minecraft/${id}/files?path=${encodeURIComponent(p)}`,
      );
      if (r.data.type === "dir") {
        setFmFiles(r.data.files);
        setFmPath(p);
        setFmContent(null);
      } else
        setFmContent({ name: p.split("/").pop() || "", text: r.data.content });
    } catch {
    } finally {
      setFmLoading(false);
    }
  ...
  const uploadWorld = async (id: string, file: File) => {
    ...
    try {
      ...
      alert("❌ " + (e.response?.data?.error || "Error al subir"));
    } finally {
  ...
  const deleteFMEntry = async (id: string, p: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!confirm(`¿Eliminar "${p.split("/").pop()}"? No se puede deshacer.`))
      return;
    try {
      await api.delete(`/minecraft/${id}/files?path=${encodeURIComponent(p)}`);
      loadFiles(id, fmPath);
    } catch {}
  };
      ...
      >
        ...
        >
          <div>
            ...
            >
              {servers.filter((s) => isRunning(s)).length} online ·{" "}
              {servers.filter((s) => !isRunning(s)).length} offline · stats cada
              5s
            </p>
          </div>
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              fontFamily: "monospace",
              boxShadow: "0 4px 12px rgba(35,134,54,.3)",
            }}
          >
            + Nuevo Servidor
          </button>
        ...
        {loading ? (
            ...
            {[1, 2].map((i) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: "12px",
                  background:
                    "linear-gradient(90deg,#161b22 25%,#1f2937 50%,#161b22 75%)",
          ...
          >
            ...
            >
              Crea tu primer servidor Minecraft Vanilla
            </p>
            <button
              className="mc-btn"
              onClick={() => setShowCreate(true)}
          ...
          >
            {servers.map((s, i) => {
              ...
              const cpuColor =
                cpuPct > 80 ? "#f87171" : cpuPct > 50 ? "#fbbf24" : "#34d399";
              const memColor =
                memPct > 85 ? "#f87171" : memPct > 60 ? "#fbbf24" : "#60a5fa";

              return (
                <div
                  key={s.id}
                  className="mc-card fade-up"
                  style={{
                ...
                >
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: "10px",
                          borderRadius: "50%",
                          background: running ? "#34d399" : "#f87171",
                          boxShadow: running
                            ? "0 0 10px #34d399"
                            : "0 0 8px #f87171",
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          ...
                          >
                            {fmt(stats.memUsage)} / {fmt(stats.memLimit)}
                          </span>
                        </div>
                        <div
                          style={{
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            Jugadores{" "}
                            {pl ? `${pl.players.length}/${pl.maxPlayers}` : "—"}
                          </div>
                          {pl && pl.players.length > 0 ? (
                            ...
                            >
                              {pl?.ready ? "Nadie online" : "Cargando..."}
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    <button
                      className="mc-btn"
                      onClick={() => navigate(`/minecraft/${s.id}`)}
                      title="Abrir panel del servidor"
                      style={{
                        padding: "9px 14px",
                        background:
                          "linear-gradient(135deg,rgba(88,166,255,.15),rgba(88,166,255,.08))",
                        color: "#58a6ff",
                    ...
                    {!running ? (
                      ...
                      <>
                        ...
                        <button
                          className="mc-btn"
                          disabled={actionId === `${s.id}-restart`}
                ...
                >
                  <div>
                    ...
                    <select
                      value={form.memoria}
                      onChange={(e) =>
                        setForm((f) => ({ ...f, memoria: e.target.value }))
                      }
                      style={{ ...inp, cursor: "pointer" }}
                    >
                      {MC_MEMORY.map((m) => (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={form.puerto}
                      onChange={(e) =>
                        setForm((f) => ({ ...f, puerto: e.target.value }))
                      }
                      style={inp}
                    />
                  </div>
        ...
        {logs && (
          <div
            className="overlay"
            onClick={(e) => e.target === e.currentTarget && setLogs(null)}
            style={{
              position: "fixed",
              inset: 0,
        ...
        {filesMgr && (
          <div
            className="overlay"
            onClick={(e) => e.target === e.currentTarget && setFilesMgr(null)}
            style={{
              position: "fixed",
              inset: 0,
              background: "rgba(0,0,0,.9)",
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    ...
                    {fmPath && (
                      <span style={{ color: "#60a5fa" }}>/{fmPath}</span>
                    )}
                  </div>
                </div>
                {/* Subir mundo ZIP */}
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      e.target.files?.[0] &&
                      uploadWorld(filesMgr, e.target.files[0])
                    }
                  />
                </label>
                {/* Descargar mundo */}
                <button
                  onClick={() =>
                    window.open(
                      `/api/minecraft/${filesMgr}/download-world`,
                      "_blank",
                    )
                  }
                  style={{
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            fontFamily: "monospace",
                            fontSize: "13px",
                            color: f.type === "dir" ? "#60a5fa" : "#e6edf3",
                            overflow: "hidden",
                            textOverflow: "ellipsis",
                            whiteSpace: "nowrap",
                          }}
                        ...
                        <button
                          onClick={(e) => deleteFMEntry(filesMgr!, f.path, e)}
                          style={{
                            background: "none",
                            border: "none",
                            color: "#374151",
